"""Independent DESI spectroscopy collector definition. Get_data performs no cross-match/classification."""
from pathlib import Path
import pandas as pd
CATALOG="DESI spectroscopy"
FIELDS=["targetid","ra","dec","survey","program","healpix","spectype","subtype","z","zerr","zwarn","deltachi2"]
...
def fetch(ra:float,dec:float,radius_arcmin:float)->pd.DataFrame:
 raise NotImplementedError("official DESI spectroscopy query endpoint/client wiring pending")

def save(df:pd.DataFrame,path:str|Path)->None:
 if not {"catalog","catalog_object_id","object_name","ra","dec"}.issubset(df.columns):raise ValueError("invalid DESI spectroscopy output")
