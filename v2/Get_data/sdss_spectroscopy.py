"""Independent SDSS spectroscopy collector definition. Get_data performs no cross-match/classification."""
from pathlib import Path
import pandas as pd
CATALOG="SDSS spectroscopy"
FIELDS=["specobjid","bestobjid","ra","dec","plate","mjd","fiberid","class","subclass","z","zerr","zwarning","snmedian"]
...
def fetch(ra:float,dec:float,radius_arcmin:float)->pd.DataFrame:
 raise NotImplementedError("official SDSS spectroscopy query endpoint/client wiring pending")

def save(df:pd.DataFrame,path:str|Path)->None:
 if not {"catalog","catalog_object_id","object_name","ra","dec"}.issubset(df.columns):raise ValueError("invalid SDSS spectroscopy output")
