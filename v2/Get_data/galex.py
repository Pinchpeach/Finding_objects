"""Independent GALEX AIS cone-search collector via VizieR. No cross-match/classification."""
from pathlib import Path
import pandas as pd
CATALOG="GALEX AIS"; VIZIER_CATALOG="II/335/galex_ais"
def fetch(ra:float,dec:float,radius_arcmin:float)->pd.DataFrame:
 ...
 if "objid" not in df.columns: raise KeyError("catalog identifier column missing")
 df.insert(0,"catalog",CATALOG);df.insert(1,"catalog_object_id",df["objid"].astype("string"))
