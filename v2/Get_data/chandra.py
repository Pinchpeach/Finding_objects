"""Independent Chandra collector definition. Get_data performs no cross-match/classification."""
from pathlib import Path
import pandas as pd
CATALOG="Chandra"
FIELDS=["source_id","source_name","ra","dec","poserr","broad_flux","broad_flux_err","soft_flux","medium_flux","hard_flux","significance","extent"]
...
def fetch(ra:float,dec:float,radius_arcmin:float)->pd.DataFrame:
 raise NotImplementedError("official Chandra query endpoint/client wiring pending")

def save(df:pd.DataFrame,path:str|Path)->None:
 if not {"catalog","catalog_object_id","object_name","ra","dec"}.issubset(df.columns):raise ValueError("invalid Chandra output")
