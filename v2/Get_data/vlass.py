"""Independent VLASS collector definition. Get_data performs no cross-match/classification."""
from pathlib import Path
import pandas as pd
CATALOG="VLASS"
FIELDS=["source_id","source_name","ra","dec","peak_flux","peak_flux_err","total_flux","total_flux_err","local_rms","maj","min","pa"]
...
def fetch(ra:float,dec:float,radius_arcmin:float)->pd.DataFrame:
 raise NotImplementedError("official VLASS query endpoint/client wiring pending")

def save(df:pd.DataFrame,path:str|Path)->None:
 if not {"catalog","catalog_object_id","object_name","ra","dec"}.issubset(df.columns):raise ValueError("invalid VLASS output")
