"""Independent FIRST cone-search collector via VizieR. No cross-match/classification."""
from pathlib import Path
import pandas as pd
CATALOG="FIRST"; VIZIER_CATALOG="VIII/92/first14"
def fetch(ra:float,dec:float,radius_arcmin:float)->pd.DataFrame:
 ...
 if "FIRST" not in df.columns: raise KeyError("catalog identifier column missing")
 df.insert(0,"catalog",CATALOG);df.insert(1,"catalog_object_id",df["FIRST"].astype("string"))
