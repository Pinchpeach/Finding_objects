"""Independent FIRST collector (radio measurements). No cross-match/classification in Get_data."""
from pathlib import Path
import pandas as pd
CATALOG="FIRST"
FIELDS=["source_id","source_name","ra","dec","peak_flux","int_flux","rms","maj","min","pa"]
...
def fetch(ra:float,dec:float,radius_arcmin:float)->pd.DataFrame:
    ...
    raise NotImplementedError("Wire the official FIRST query endpoint/client here")

def save(df:pd.DataFrame,path:str|Path)->None:
    if not {"catalog","catalog_object_id","object_name","ra","dec"}.issubset(df.columns): raise ValueError("invalid FIRST output")
